...
function EditService({ id, setId }) {
  ...
  function submitForm(e) {
    setLoader(true)
    e.preventDefault();
    let newData = { title: title, description: desc, cover: base64, owner: data.owner, ownerId: data.ownerId }
    axios.patch(Base_Url + `/api/services/${id}`, newData, {
      headers: {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          setId(null)
          handleReset()
          let index = store.services.filteredData.findIndex((e) => e.id == id)
  ...
  return (
    <>
      <div onClick={() => { store.serviceEdit.setData(false) }}
        style={store.serviceEdit.data ? {} : { display: 'none' }}
        className={styles.glass}>
      </div>

      <div style={store.serviceEdit.data ? {} : { height: "0", padding: '0' }} className={styles.modal}>
        <form className={styles.form} onSubmit={(e) => { submitForm(e) }}>
          ...
          <div className={styles.btns}>
            <Button className={styles.btn} color='error' variant="contained" onClick={() => { setId(null), store.serviceEdit.setData(false) }}>
