...
function AddService() {
  ...
  function submitForm(e) {
    ...
    if (!title) setError("Title is required!");
    else if (!desc) setError("Title is required!");
    else if (!base64) setError("Title is required!");
    else {
      let newData = {
        title: title,
        description: desc,
      ...
      axios
        .post(Base_Url + "/api/services", newData, {
          headers: {
  ...
  return (
    <>
      <div
        onClick={() => {
          store.serviceAdd.setData(false);
        }}
      ...
      <div
        style={store.serviceAdd.data ? {} : { height: "0", padding: "0" }}
